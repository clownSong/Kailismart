$(function () {
    ...
    var putRes = function (res) {
        $(res).each(function (index, element) {
            if (this.approveType == 0) {      //未审核
                this.operation = '<button class="btn btn-danger btn-xs delete" type="button">删除</button>' +
                    '<button class="btn btn-success btn-xs approve" data-approve="' + this.approveType + '" type="button">审核</button>' +
                ...
                this.operation = '<button class="btn btn-danger btn-xs delete" disabled="disabled" type="button">删除</button>' +
                    '<button class="btn btn-success btn-xs approve" data-approve="' + this.approveType + '" type="button">反审核</button>' +
                    '<button class="btn btn-success btn-xs print" type="button">打印</button>';
    ...
    //默认加载本月订单
    var table = serverTable("getPutList?index=1&start=" + getfirstDate(new Date()) + "&end=" + getEndDate(new Date()) + "", {
    ...
    table.bootstrapTable("refreshOptions", {
        onDblClickRow: function (row, $element, field) {
            putObject = row;
            $(putStorage.pro).text(putObject.procurement.pmNumber);     //订单号赋值
            $(putStorage.company).text(putObject.company.name);         //供应单位赋值
            $(putStorage.put).val(putObject.putSerial);                //入库单编号赋值
            $(putStorage.tax).val(putObject.tax);              //税率赋值
            $(putStorage.mixMoney).val(putObject.mixMoney);              //运杂费赋值
            $(putStorage.remark).val(putObject.remark);              //备注赋值
            $(putStorage.putDate).val(putObject.putDate);              //入库时间
            ...
            if (materialTable != undefined) {
                $(materialTable).bootstrapTable(("refresh"), {url: "/managent/getMaterByPutId?putId=" + row.iD});
            }
            materialTable = initTable($("#putMaterial"), "/managent/getMaterByPutId?putId=" + row.iD, {}, function (materials) {
                putObject.materialList = materials;
    ...
    $("#applys").on("click", ".delete,.approve,.print", function () {
        ...
        } else if ($(this).is(".print")) {  //删除
            var temp = $(table).bootstrapTable("getRowByUniqueId", $(this).closest("tr").data("uniqueid"));
            appendPrint(temp);
        }
    });
//注册模态框pass按钮事件
...
//注册输入框失去焦点事件
    ...
    var filter = {approveType: undefined, approveType: undefined, tempPutState: undefined};
//注册筛选按钮事件
        ...
        switch ($(this).attr("id")) {
            case "select-approve":      //已审核或未审核
                if (value == "已审核") {
                    ...
                    $(this).text("未审核");
                    filter.approveType = 0;
                } else if (value == "未审核") {
                    $(this).css("background-color", "#5bc0de");
                    $(this).text("未|审");
                    filter.approveType = undefined;
                } else {
                    $(this).css("background-color", "rgba(91, 192, 222, 0.72)");
                    $(this).text("已审核");
                    filter.approveType = 1;
                }
                break;
        }
        temp = filter;
...
//模态框关闭事件
    ...
    $(".dropdown").on("click", "li", function () {
        ...
        if ($ul.attr("aria-labelledby") == "selectDate") {    //根据时间加载数据
            ...
            $(table).bootstrapTable("refresh", {url: 'getPutList?index=1&start=' + start + "&end=" + end});
        } else {      //流程选择后的处理
    ...
    function approvePut(id, btn) {
        ...
        $.ajax({
            url: "http://" + webIp + "/managent/approvePut",
            data: JSON.stringify(temp),
            type: 'post',
            contentType: "application/json;charset=utf-8",
            success: function (mess) {
                if (mess != -1) {
                    switch (mess.approveType) {
                        case 1:
                            temp.approveType = 1;
    ...
    function updatePutStorage() {
        return $.ajax({
            ...
            contentType: "application/json;charset=utf-8",
            success:function () {
                updateRow(putObject.index,putObject);
            }
        })
    }
    ...
    function appendPrint(putObject) {
        ...
        $("#putNumber").children(0).text(putObject.putSerial);
        $("#proNumber").children(0).text(putObject.procurement.pmNumber);
        $("#storage").children(0).text(putObject.storage.name);
        $("#company_print").children(0).text(putObject.company.name);
        $("#print_data").children(0).text(putObject.putDate);
        $("#print_person").children(0).text(putObject.staff.name);
        $("#print_remark").children(0).text(putObject.remark);
        if(putObject.materialList == undefined){
            $.ajax({
                url:'http://'+webIp+'/managent/getMaterByPutId',
                type:'post',
                data:'putId='+putObject.iD,
                success:function (mater) {
                    putObject.materialList = mater;
                    ...
                    $("#moneyCount").children(0).text(taxSum.toFixed(4));
                    $("#moneyCount").next().children(0).text(sum.toFixed(4));
                    $("#moneyCount").prev().children(0).text(formatRMB(taxSum));
